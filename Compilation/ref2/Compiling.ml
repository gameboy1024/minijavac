...
let addFun cname fname fbody args env = 
  ...
  let fid = ComplingEnv.getFunID cname fname in
  let f = ComplingEnv.makeFun fargs fbody in
  ComplingEnv.addFun env fid f
...
let rec makeFuns cname funs env =
  match funs with
    | [] -> env
    | f::others ->
      ComplingEnv.addFunToClass cname f.mname env;
...
let rec makeClassEnv typesAST env = 
  match typesAST with
    | [] -> env
    | c::others ->
      let newClass = (ComplingEnv.makeClass c.cname c.cparent) in
      (* On ajoute la classes à l'environnement *)
      let envWithClasses = ComplingEnv.addClass env c.cname newClass in
      let cattrs = makeAttrs envWithClasses c.cattributes in
      ComplingEnv.addAttrsToClass c.cname cattrs envWithClasses ;
...
let compile typesAST =
  makeClassEnv typesAST (ComplingEnv.initialEnv())
